import type { Quotation, QuotationItem, QuotationStatus } from '@/models/types';
...
const sampleItems: QuotationItem[] = [
  {
    productId: 1,
    productName: 'Broadband Plan - 50 Mbps',
    quantity: 1,
    unitPrice: 499,
    taxRate: 18,
    discount: 0,
    lineTotal: 499 + (499 * 18) / 100,
  },
];
...
let quotationsData: Quotation[] = [
  {
    ...
    taxTotal: (499 * 18) / 100,
    discountAmount: 0,
    grandTotal: 499 + (499 * 18) / 100,
    status: 'sent',
    validUntil: getDate(7),
    notes: 'Standard broadband quotation valid for 7 days.',
    createdAt: getDate(-1),
  },
  {
    id: 2,
    ...
    items: [
      {
        productId: 2,
        productName: 'POS License - Retail',
        quantity: 1,
        unitPrice: 1999,
        taxRate: 18,
        discount: 10,
        lineTotal: 1999 * 0.9 * 1.18,
      },
    ],
    subtotal: 1999,
    taxTotal: (1999 * 0.9 * 0.18),
    discountAmount: 199.9,
    grandTotal: 1999 * 0.9 * 1.18,
    status: 'draft',
    validUntil: getDate(14),
    notes: 'Includes 10% promotional discount for POS license.',
    createdAt: getDate(0),
  },
];
...
export const quotationsApi = {
  getAll: async (): Promise<Quotation[]> => {
    return Promise.resolve([...quotationsData]);
  },
  ...
  create: async (
    quotation: Omit<Quotation, 'id' | 'createdAt' | 'quotationNumber'>
  ): Promise<Quotation> => {
    const newQuotation: Quotation = {
      ...quotation,
      organizationId: quotation.organizationId ?? MOCK_ORGANIZATION_ID,
      id: nextId++,
