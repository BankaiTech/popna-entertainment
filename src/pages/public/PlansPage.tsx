...
const PlansPage = () => {
  ...
  const [selectedFilter, setSelectedFilter] = useState<string>(productName || 'All');
  useEffect(() => {
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPlans.map((plan) => {
              ...
              return (
                <Card key={plan.id} className="flex flex-col h-full hover:shadow-xl transition-all duration-300 border border-gray-200">
                  <CardHeader className="flex-shrink-0">
                    <CardTitle className="text-xl text-gray-900 mb-2">{plan.planName}</CardTitle>
                  ...
                  <CardContent className="flex-1 flex flex-col">
                    ...
                    <div className="mt-auto pt-4">
                      <Link to={`/${plan.provider.toLowerCase().includes('gtpl') ? 'cable' : 'internet'}/${plan.provider.toLowerCase()}`}>
